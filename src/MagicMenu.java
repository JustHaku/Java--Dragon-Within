...
public class MagicMenu extends Menu implements State {
    ...
    private ArrayList<Text> magicText = new ArrayList<>();
    ...
    public MagicMenu(RenderWindow window, int scale, int options_num, ArrayList<Character> team) throws IOException {
      ...
      for (int i = 0; i < 4; i++) {
            ...
            magicText.add(new Text("" + i, text_font, screenHeight / 15));
            (magicText.get(i)).setPosition(15 + StateMachine.xOffset, 5 + ((screenHeight / 4 - 2) * i) + StateMachine.yOffset);
        }
    ...
    * Main loop draws and controlls moving through menu.
    ...
    public int run() {
        ...
        while (window.isOpen() && paused == false) {
            ...
            magicText = new ArrayList<>();
            //magic = team.get(selected).magic;

            // Uncoment when magic are implimented.
            for (int i = 0; i < 4; i++) {
              //magicText.add(new Text(magic[i].getName(), text_font, screenHeight / 15));
              //magicText.get(i).setPosition(15, 5 + ((screenHeight / 4 - 2) * i));
              window.draw(skillsRect.get(i));
              //window.draw(magicText.get(i));
            }
